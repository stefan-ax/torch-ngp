...
if __name__ == '__main__':
    to_write_lines = [
        ...
        "# Number of images: 100, mean observations per image: 1105.6300000000001"
    ]
    ...
    for i in tqdm(range(N-1)):
        ...
        # R = rot3to4(R.as_matrix())
        # T_inv = np.array([[1.0, 0.0, 0.0, NEW_TX],
        #                   [0.0, 1.0, 0.0, NEW_TY],
        #                   [0.0, 0.0, 1.0, NEW_TZ],
        #                   [0.0, 0.0, 0.0, 1.0]])
        # T     = np.array([[1.0, 0.0, 0.0, -NEW_TX],
        #                   [0.0, 1.0, 0.0, -NEW_TY],
        #                   [0.0, 0.0, 1.0, -NEW_TZ],
        #                   [0.0, 0.0, 0.0, 1.0]])
        
        # # new_transform = T_inv @ (R @ T)
        # # new_transform = T_inv.T @ R
        # new_transform = R.T
        
        # new_transform_quat = Quaternion(matrix=new_transform)
        # new_transform_quat = Quaternion(matrix=np.eye(3))
        new_transform_quat = Quaternion(matrix=R.as_matrix().T)
        NEW_T = -R.as_matrix().T.dot(NEW_T)
        NEW_TX, NEW_TY, NEW_TZ = NEW_T
        
        # if theta != 0.0:
        #     # new_transform_quat = Quaternion(axis=[0, 1/np.cos(theta), 0], angle=np.pi/2) * Quaternion(axis=[np.sin(theta), np.cos(theta), 0], angle=-np.pi/2)
        #     # new_transform_quat = Quaternion(axis=[np.sin(theta), np.cos(theta), 0], angle=np.pi/2)# * Quaternion(axis=[0, 0, 1], angle=np.pi/10)
        #     # new_transform_quat = Quaternion(axis=[0, 0, 0], angle=np.pi/2)
        #     new_transform_quat = Quaternion(matrix=Rotation.from_euler('X', [np.pi/2]).as_matrix()[0])
        # else:
        #     new_transform_quat = Quaternion(axis=[1, 0, 0], radians=np.pi/2)
